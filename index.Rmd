...
### Tempogram {data-commentary-width=500}
...
nexus_plot <- nexus %>%
  ...
  theme_vapor()
ggnexus <- ggplotly(nexus_plot)

hang <- get_tidy_audio_analysis("5PlZbtB1Ok46sKyjKEfppR")
hang_plot <- hang %>%
  tempogram(window_size = 8, hop_size = 1, cyclic = TRUE) %>%
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
  scale_fill_viridis_c(guide = "none") +
  labs(x = "Time (s)", y = "Tempo (BPM)", title = "Looking") +
  theme_vapor()
gghang <- ggplotly(hang_plot)
```

```{r echo=FALSE}
subplot(ggnexus, gghang, nrows = 2, margin = 0.05, shareX = TRUE, titleY = TRUE) %>% layout(title = '') %>% 
  layout(annotations = 
...
Energy and Valence can convey the mood of songs. We can see clearly that almost all songs have 0.5 or more energy, with Crockett's Theme being the lowest at 0.38 energy. It also seems that Major key songs have lower valence then Minor key songs, but this trend is not that pronounced.
...
subplot(ggplotly(daysofthunder_plot), ggplotly(wildones_plot), nrows = 2, margin = 0.05, shareX = TRUE) %>% 
  layout(title ='') %>% 
  layout(annotations = 
