...
pr <- synth_forest %>% get_pr() %>% 
  rename(Precision = precision, Recall = recall)%>% 
  pivot_longer(c("Precision", "Recall"), names_to = "type", values_to = "percentage")

p <- ggplot(pr, aes(class, percentage, fill = type)) + 
  geom_bar(stat="identity", position = "dodge") + 
  labs(
  x="Artist",
  y="Percentage",
  title="Statistics per label",
  caption="Source: Spotify API",
  fill = "Statistic                ") +
  theme_vapor()
